...
void Model::Draw(Shader &shader, glm::mat4 &model) {
  for (unsigned int i = 0; i < meshes.size(); i++) {
    ...
    glm::mat3 normalMatrix =
        glm::transpose(glm::inverse(glm::mat3(currentModel)));
...
void Model::loadModel(std::string path) {
  ...
  preloadTextures(scene); // decodifica todas as imagens em paralelo antes de
                          // montar as meshes
...
void Model::preloadTextures(const aiScene *scene) {
  ...
  for (auto &f : futures)
    f.wait(); // espera todas terminarem antes de seguir pro processamento das
              // meshes
}
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
  ...
  glm::vec4 solidColor(diffuseColor.r, diffuseColor.g, diffuseColor.b,
                       diffuseColor.a);
...
void Model::getCollisionData(JPH::VertexList &outVertices,
  ...
  for (unsigned int m = 0; m < meshes.size(); m++) {
    ...
    // TODO: verificar se o mesh tem flag de colisao
    //  aplica a transform do nó em cada vértice, igual fizemos no
    //  getBoundingRadius
    for (const auto &vertex : meshes[m].vertices) {
      glm::vec4 worldPos = meshTransforms[m] * glm::vec4(vertex.Position, 1.0f);
    ...
    for (size_t i = 0; i + 2 < indices.size(); i += 3) {
      outTriangles.push_back(JPH::IndexedTriangle(baseIndex + indices[i],
