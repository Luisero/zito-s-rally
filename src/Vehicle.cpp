...
Vehicle::Vehicle(PhysicsManager *physicsManager, glm::vec3 startPos, Model *chassisModel)
{
    ...
    OffsetCenterOfMassShapeSettings com_settings(Vec3(0.0f, -0.3f, 0.0f), box_shape);
    ...
    BodyCreationSettings chassis_settings(chassis_shape, Vec3(startPos.x, startPos.y, startPos.z), Quat::sIdentity(), EMotionType::Dynamic, Layers::MOVING);
    chassis_settings.mMassPropertiesOverride.mMass = 1500.0f; // 1500 kg
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        wheel->mPosition = Vec3(
            is_left ? -half_vehicle_width : half_vehicle_width,
            -0.2f,
    ...
    controller_settings->mEngine.mMaxRPM = 6000.0f;

    
    controller_settings->mTransmission.mMode = JPH::ETransmissionMode::Auto;
    controller_settings->mTransmission.mGearRatios= { 2.66f, 1.78f, 1.3f, 1.0f, 0.74f };
    ...
    JPH::BodyLockWrite lock(physicsManager->physics_system->GetBodyLockInterface(), chassisId);
    if (lock.Succeeded())
    {
        JPH::Body& chassis = lock.GetBody();
        vehicleConstraint = new JPH::VehicleConstraint(chassis, vehicle_settings);
        
       // vehicleConstraint->SetVehicleCollisionTester(new VehicleCollisionTesterRaycast(Layers::MOVING));
        vehicleConstraint->SetVehicleCollisionTester(new VehicleCollisionTesterRay(Layers::MOVING));
...
void Vehicle::setInput(float forward, float right, float brake, float handBrake)
{
     JPH::BodyLockWrite lock(physicsManager->physics_system->GetBodyLockInterface(), chassisId);
    if (lock.Succeeded())
    {
        JPH::Body& chassis = lock.GetBody();
        WheeledVehicleController* controller = static_cast<WheeledVehicleController*>(vehicleConstraint->GetController());
