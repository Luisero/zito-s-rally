...
void Game::setupWindow() {
  ...
  window.create(desktop, "Zito's Rally", sf::Style::Fullscreen,
                contextSettings);
  bool contextReady = false;
  for (int attempt = 0; attempt < 10 && !contextReady; ++attempt) {
    contextReady = window.setActive(true);
    if (!contextReady) {
      std::cerr << "Context not active yet, retrying... (" << attempt << ")"
                << std::endl;
  ...
  if (!contextReady) {
    std::cerr << "FATAL: Could not activate OpenGL context after retries"
              << std::endl;
